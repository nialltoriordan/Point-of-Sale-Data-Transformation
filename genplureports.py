...
"""
Seem to be getting weird formatting errors when opening with openoffice.  Going to try a lazy work-around.

Suspect error due to type errors - numbers being stored as strings.  The read_csv function automatically changes types.
"""
time_data = pd.read_csv(outputFile, header=0, index_col=False) 

time_data['Date'] = pd.to_datetime(time_data['Date'], dayfirst = True)
time_data.sort_values(by='Date', ascending = True)   #this doesn't seem to work properly - causes errors when a week spans two months
#reset index
time_data = time_data.reset_index(drop=True) 

time_data.to_csv(outputFile, index=False)

"""
Work-around seems to work.  Shelving issue for now.
"""
